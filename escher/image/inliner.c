...
void generateComposedImplementationFromImage(FILE * file, const char * header, const char * variable, uint32_t width, uint32_t height, png_byte colorType, png_bytep * pixelsRowPointers, int blockMaxHeight) {
  ...
  for (int block=0; block < blockNumber; block++)
  {
    ...
    fprintf(file, "const uint8_t compressedPixelData%d[%d] = {", block, sizeOfCompressedPixelBuffer);
    for (int i=0; i<sizeOfCompressedPixelBuffer; i++) {
      fprintf(file, "0x%04x, ", compressedPixelBuffer[i]);
    }
    ...
    fprintf(file, "\n};\n\n");
    fprintf(file, "Image image%d = Image(%d, %d, compressedPixelData%d, %d);\n\n", block, width, blockHeight, block, sizeOfCompressedPixelBuffer);
  }

  fprintf(file, "const Image images[]={");
  for (int i = 0; i < blockNumber; i++) {
    fprintf(file, "image%d, ", i);
  }
  fprintf(file, "};\n");

  fprintf(file, "constexpr ComposedImage image = ComposedImage(%d, %d, images, %d);\n\n",width, height, blockNumber);
  

  fprintf(file, "const ComposedImage * ImageStore::%s = &image;\n\n", variable);
}