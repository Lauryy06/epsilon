...
void generateComposedHeaderFromImage(FILE * file, const char * guardian, const char * variable);
void generateComposedImplementationFromImage(FILE * file, const char * header, const char * variable, uint32_t width, uint32_t height, png_byte colorType, png_bytep * pixelsRowPointers, int blockMaxHeight);
...
int main(int argc, char * argv[]) {
  ...
  */
  ...
  if(strstr(upperSnakeCaseName, "COMPOSED")) {
    int blockHeight = 12;//TODO: Replace this by ComposedImage::k_blockHeight 

    FILE * header = fopen(headerPath, "w");
    generateComposedHeaderFromImage(header, upperSnakeCaseName, camelCaseName);
    fclose(header);

    FILE * implementation = fopen(implementationPath, "w");
      
    generateComposedImplementationFromImage(implementation, lowerSnakeCaseName, camelCaseName, width, height, colorType, rowPointers, blockHeight);
...
void generateComposedHeaderFromImage(FILE * file, const char * guardian, const char * variable)
{
  ...
  fprintf(file, "#ifndef IMAGE_STORE_%s_H\n", guardian);
  fprintf(file, "#define IMAGE_STORE_%s_H\n\n", guardian);
  fprintf(file, "#include <escher/composed_image.h>\n\n");
  fprintf(file, "namespace ImageStore {\n\n");
  fprintf(file, "extern const ComposedImage * %s;\n\n", variable);
  fprintf(file, "};\n\n");
  fprintf(file, "#endif\n");
}
...
void generateComposedImplementationFromImage(FILE * file, const char * header, const char * variable, uint32_t width, uint32_t height, png_byte colorType, png_bytep * pixelsRowPointers, int blockMaxHeight) {
  ...
  //For unknown reasons, the linker does not work if all the compressed data are not in the same array 

  fprintf(file, "static const uint8_t compressedPixelData[] = {");

  int blockNumber = ceil(1.0*height/blockMaxHeight);
  int compressedPixelDataSizes[blockNumber];
  int blockHeights[blockNumber];

  for (int block=0; block < blockNumber; block++)
  {
    int blockHeight = (block+1)*blockMaxHeight<=height ? blockMaxHeight : height-(block*blockMaxHeight);

    int sizeOfPixelBuffer = width * blockHeight * sizeof(uint16_t);
    uint16_t * pixelBuffer = (uint16_t *)malloc(sizeOfPixelBuffer);
    
    for (int j=0; j<blockHeight; j++) {
      png_bytep pixelRow = pixelsRowPointers[(block*blockMaxHeight)+j];
      for (int i=0; i<width; i++) {
        double alpha;
        png_bytep pixel;
        if (colorType == PNG_COLOR_TYPE_RGB) {
    ...
    fprintf(file, "\n   // Image : %d, Compressed %d pixels into %d bytes (%.2f%% compression ratio)/\n", block, width*blockHeight, sizeOfCompressedPixelBuffer, 100.0*sizeOfCompressedPixelBuffer/sizeOfPixelBuffer);
    ...
    compressedPixelDataSizes[block] = sizeOfCompressedPixelBuffer;
  ...
  {
    fprintf(file, "static const Image image%d = Image(%d, %d, compressedPixelData+%d, %d);\n", i, width, blockHeights[i], offset, compressedPixelDataSizes[i]);
    offset += compressedPixelDataSizes[i];
  }

  fprintf(file, "\nstatic const Image * images[]={");

  for (int i = 0; i < blockNumber; i++) {
    fprintf(file, "&image%d, ", i);
  }

  fprintf(file, "};\n\n");

  fprintf(file, "static constexpr ComposedImage finalImage = ComposedImage(%d, %d, images, %d);\n\n",width, height, blockNumber);

  fprintf(file, "const ComposedImage * ImageStore::%s = &finalImage;\n\n", variable);
}